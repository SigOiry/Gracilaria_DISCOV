...
(p1  <-
  ...
  theme(
    ...
    panel.grid.minor = element_line(color = "grey", linetype = "dotted", linewidth = 0.25)  # Explicitly add grid
  )
  )
...
ext_plot_Saja <- st_bbox(create_square_sf(5000, c(-4.025987, 43.4250065)))
...
(plot_Saja <- ggplot() +
  ...
  geom_sf(data = It_Saja, linewidth = 0.05, alpha = 0.93, fill = "goldenrod")+
    geom_label(aes(x = as.numeric(ext_plot_Saja[1]) + (as.numeric(ext_plot_Saja[3])-as.numeric(ext_plot_Saja[1]))*0.9 , 
                   y = as.numeric(ext_plot_Saja[2]) + (as.numeric(ext_plot_Saja[4])-as.numeric(ext_plot_Saja[2]))*0.9
                   , label = "C"), 
               size = 10,
               alpha = 0.5 )+
  coord_sf(
